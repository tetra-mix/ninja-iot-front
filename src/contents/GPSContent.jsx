...
export const GPSContent = () => {
    ...
    const GPSDetect = () => {
        ...
        const new_distance = getDistance(oniLatRef.current, oniLngRef.current, sinobiLatRef.current, sinobiLngRef.current);
        console.log(new_distance);
        if(isNaN(new_distance)){
            setMessage("距離: " + "計測不能");
        }else{
            setMessage("距離: " + FloatToString(new_distance) + " m");
        }
        ...
        if (new_distance <= 10) {
            NumToDetect(4);
        } else if (new_distance <= 20) {
            NumToDetect(3);
        } else if (new_distance <= 30) {
            NumToDetect(2);
        } else if (new_distance <= 40) {
            NumToDetect(1);
        } else {
            NumToDetect(0);
        }
    ...
    function getDistance(latitude0,longitude0, latitude1,longitude1){
        ...
        var _distance = Math.sqrt(difLatM * difLatM + difLngM * difLngM);
        return _distance; // in meter
    }
        ...
        <Box>
            ...
            <Box text="lg" mt={2} p={4} borderRadius={"2xl"} border={"solid"} borderColor="success.500">
                ...
                <Flex w="full" gap="sm">
                    ...
                    <Text >緯度:{oniLat == 0 ? "無し": oniLat}</Text>
                    <Text >経度:{oniLng == 0 ? "無し": oniLng}</Text>
                </Flex>
                <Flex w="full" gap="sm">
                    <Text pr="4" text="xl" fontWeight={"bolder"}>忍</Text>
                    <Text >緯度:{sinobiLat == 0 ? "無し": sinobiLat}</Text>
                    <Text >経度:{sinobiLng == 0 ? "無し": sinobiLng}</Text>
                </Flex>
